...
def approve_or_disapprove_user(message):
    ...
    if action == '/approve':
        if plan == 1:
            if users_collection.count_documents({"plan": 1}) >= 200:
                bot.send_message(chat_id, "*Approval failed: Instant Plan 🧡 limit reached (200 users).*", parse_mode='Markdown')
                return
        elif plan == 2:
            if users_collection.count_documents({"plan": 2}) >= 800:
                bot.send_message(chat_id, "*Approval failed: Attack limit reached (800 users).*", parse_mode='Markdown')
                return
...
def send_welcome(message):
    ...
    bot.send_message(message.chat.id, "*🔆 WELCOME TO 𝐀ÿ𝐚𝖆𝖓 𝐃𝐃𝐎𝐒 🔆*", reply_markup=markup, parse_mode='Markdown')
